 ...
 * logged out.
 */
Ext.define('sgaf.util.SessionMonitor', {
  rquires: ['sgaf.util.SessionController'],
...
//      html: "Your session will automatically expires after 15 minutes of  inactivity. If your session expires, any unsaved data will be lost and  you will be automatically logged out. </br></br>If you want  to continue working, click the 'Continue Working'  button.</br></br>"
    ...
    buttons: [{
      ...
      handler: function() {
        Ext.TaskManager.stop(sgaf.util.SessionMonitor.countDownTask);
        sgaf.util.SessionMonitor.window.hide();
        sgaf.util.SessionMonitor.start();
      ...
      handler: function() {
        ...
        var menu = Ext.ComponentQuery.query('menu#menuHeader')[0];
        menu.items.filter('itemId', 'logout').fireEvent('click', menu.items.filter('itemId', 'logout'));

        sgaf.util.SessionController.logout();
        if(!sessionUsuario.getLocal()) {
          window.location.reload();
        }else{
          sgaf.util.Util.showErrorMsg('Ocorreu um problema!');
        }
      }
    }]
  }),
